...
const ShowCauseNotices = () => {
  ...
  const [sendTermsReminder, { isLoading: reminderLoading }] =
    useSendTermsReminderMutation();
  ...
  const getTermsStatus = (employeeId) => {
    ...
    const employeeTerms = termsStatusData.data.find(
      (emp) => emp.employeeId === employeeId
    );
    return (
      employeeTerms?.termsStatus || {
        submitted: false,
        submittedAt: null,
        version: "1.0",
      }
  ...
  const handleSendReminder = async (employeeId, employeeName) => {
    try {
      ...
      toast.success(
        result.message ||
          `Reminder sent to ${employeeName} for terms and conditions submission`
      );
    } catch (error) {
    ...
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="text-xl text-gray-600">
          Loading employees and terms data...
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold">
                Employee Terms & Conditions Status
              </h1>
              <p className="text-blue-100 mt-1">
                Track which employees have submitted their terms and conditions
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {termsStatusData?.data?.filter(
                    (item) => item?.termsStatus?.submitted
                  )?.length || 0}
                </p>
              </div>
            </div>
