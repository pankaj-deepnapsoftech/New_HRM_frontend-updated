...
const EmployeeForm = ({ showForm, setShowFrom, editTable }) => {
  ...
  const {
    ...
    initialValues: {
      photo: editTable?.photo || "",
      pancard: editTable?.pancard || "",
      aadhaar: editTable?.aadhaar || "",
      Driving_Licance: editTable?.Driving_Licance || "",
      Voter_Id: editTable?.Voter_Id || "",
      UAN_number: editTable?.UAN_number || "",
      Back_Name: editTable?.Back_Name || "",
      Bank_Account: editTable?.Bank_Account || "",
      IFSC_Code: editTable?.IFSC_Code || "",
      Bank_Proof: editTable?.Bank_Proof || "",
  ...
  return (
    <section
      ...
      } fixed inset-0 bg-black/40 flex items-start justify-center pt-10 transition-opacity duration-500 ease-in-out z-50`}
    >
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <select
            ...
            onChange={(e) => {
              ...
              const selectedEmp = EmpData?.data?.find(
          ...
          >
            <option className="text-gray-400">Select a user</option>
