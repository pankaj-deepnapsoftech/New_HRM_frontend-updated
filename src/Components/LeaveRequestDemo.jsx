...
const LeaveRequestDemo = () => {
  ...
  } = useGetLeaveRequestByIdQuery("68d7a5caae327711cf2141b5");
  ...
  if (requestsError)
    return <div>Error loading leave requests: {requestsError.message}</div>;
  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-6">
        ...
        {leaveRequests?.data?.map((request) => (
          <div key={request._id} className="border p-4 mb-2 rounded">
            ...
            <p>
              <strong>To:</strong> {new Date(request.to).toLocaleDateString()}
            </p>
            <p>
            ...
            <div className="mt-2 space-x-2">
              <button
                onClick={() =>
                  handleUpdateRequest(request._id, { status: "approved" })
                }
      ...
      {specificError && (
        <div>Error loading specific request: {specificError.message}</div>
      )}
      {specificRequest && (
        <div className="mb-6">
          <h3 className="text-lg font-semibold mb-2">
            Specific Request (ID: 68d7a5caae327711cf2141b5)
          </h3>
          <div className="border p-4 rounded">
            <p>
              <strong>Type:</strong> {specificRequest.data?.type}
            </p>
            <p>
              <strong>From:</strong>{" "}
              {new Date(specificRequest.data?.from).toLocaleDateString()}
            </p>
            <p>
              <strong>To:</strong>{" "}
              {new Date(specificRequest.data?.to).toLocaleDateString()}
            </p>
            <p>
              <strong>Mode:</strong> {specificRequest.data?.mode}
            </p>
            <p>
              <strong>Status:</strong> {specificRequest.data?.status}
            </p>
          </div>
        </div>
      )}
